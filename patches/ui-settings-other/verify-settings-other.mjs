 ...
 *
 * Carrier shape: `webServer` (with `agents`) is a STATIC module-level `inject`
 * dependency, so the mock context exposes them as plain properties and every
 * route registers during `apply()`. The harness pins that decision both on the
 * source (no `ctx.connection`, no dynamic `ctx.inject(['webServer', …])` gate
 * inside `apply`) and at runtime (routes live as soon as `apply` returns, on
 * the plugin's own fiber, and are released in reverse order by the unload). The
 * kept `ctx.get('webServer')` fallback is checked separately: absent carrier ->
 * one warning + no registration, never a throw.
 *
 ...
 * validation (never invokes `restart` with a real script — the only spawns are
 * harness-owned stub scripts in a temp directory, which is what lets the spawn
 * smoke pin the three measured PowerShell traps). Covers the runtime-status
 * snapshot (serviceInfo / listeningPorts / dshVersion), the restart endpoint's
 * session protection (busy refusal / forced cancel), and the version endpoints
 * (`compareVersions` / `isNewerVersion` / `parseRegistry` / `resolveRegistry`
 * ordering + no token leakage, then `versionCheck` and `update` over the real
 * route). Also guards the P0 regression: `apply` must NOT return a thenable
 * (Cordis treats a returned Fiber as an invalid Effect).
 *
 * The idle auto-stop is gone (feature deleted, settings namespace included), so
 * this harness pins that removal: `status` carries no `idle` key, the host half
 * neither injects nor reads a settings service (and asks for nothing
 * dynamically at all), and it arms no interval.
 *
 ...
 * skipped with a notice. React / react-dom / jsdom and the theme bundle resolve
 * through `scripts/test-deps.mjs` (repo node_modules → $NODE_PATH → the
 * deployed profile trees), so this harness runs from any checkout.
 */
...
// The browser-side deps (react / react-dom / jsdom) resolve through the shared
// helper: the repository's own node_modules first, then $NODE_PATH, then the
// deployed profile trees. Nothing here may anchor to a machine-specific
// checkout path — that path only exists on the one machine that built it.
...
// Shape checks look at CODE only: this patch's own doc comments name the two
// rejected transports on purpose (`ctx.connection.rpc.handle` and the dynamic
// `ctx.inject([...])` carrier gate), so comments are stripped first — otherwise
// the harness would flag the very comments that document the fix.
const hostCode = readFileSync(hostPath, 'utf8')
  .replace(/\/\*[\s\S]*?\*\//g, '')
  .replace(/^\s*\/\/.*$/gm, '')

// --- the merge decision, pinned: the HTTP carrier is a STATIC dependency -----
{
  ...
  for (const dep of ['agents', 'webServer']) {
    if (!host.inject.includes(dep)) {
      throw new Error(`host.inject must declare '${dep}' as a hard dependency: ${JSON.stringify(host.inject)}`)
    }
  }
  if (/\bctx\.connection\b/.test(hostCode)) {
    throw new Error('the host half reads ctx.connection again — nothing may; the transport is a webServer route')
  }
  if (/connection\.rpc/.test(hostCode)) {
    throw new Error('the host half registers a connection.rpc channel again — that channel never exists in dsh 0.1.5 for a plugin outside the connection package')
  }
  // The regression this locks: putting the carrier back behind a dynamic
  // `ctx.inject(['webServer', …])` inside `apply` re-opens the parallel-mount
  // race (the carrier binds later than `agents`, so the callback sees
  // `undefined` and the page silently loses its transport). The optional
  // `ctx.get('webServer')` read further down is a DELIBERATE kept fallback —
  // the carrier exists because it is declared here, and the getter only lets a
  // "declared yet absent" case surface as a health warning instead of a throw.
  if (/ctx\.inject\(\s*\[[^\]]*['"]webServer/.test(hostCode)) {
    throw new Error(`the web carrier must stay a STATIC inject, not a dynamic gate inside apply: inject = ${JSON.stringify(host.inject)}`)
  }
  console.log(`host inject OK: static inject = ${JSON.stringify(host.inject)}; no ctx.connection, no dynamic webServer gate in the code`)
}
...
// Pure helpers
{
  ...
  // The update script resolves the same way (config > default under the dsh
  // home) — the `update` endpoint shares the whole restart tail with it.
  const updateDefault = host.resolveUpdateScript({})
  if (!updateDefault.toLowerCase().endsWith('.dsh\\scripts\\update-dsh.ps1')) throw new Error(`default update script: ${updateDefault}`)
  if (host.resolveUpdateScript({ updateScript: 'C:\\Custom\\update.ps1' }) !== 'C:\\Custom\\update.ps1') throw new Error('custom update script path')
  console.log('host helpers OK: resolveRestartScript + resolveUpdateScript + buildRestartSpawn + runningSessionIds')
}
...
// unparseable side must never read as "newer").
{
  ...
  // `compareVersions` is the primitive under `isNewerVersion`: -1 / 0 / 1, and
  // null when either side is not a version at all — "cannot tell" is never
  // allowed to read as "newer".
  for (const [left, right, expected] of [
    ['0.1.5-rc.2', '0.1.5-rc.3', -1],
    ['0.1.5-rc.3', '0.1.5-rc.2', 1],
    ['0.1.5', '0.1.5', 0],
    ['0.1.5', '0.1.5-rc.3', 1],
    ['0.1.5-rc.3', '0.1.5-rc.3.1', -1],
    ['0.1.5-rc.3', 'nonsense', null],
    ['nonsense', 'nonsense', null],
  ]) {
    if (host.compareVersions(left, right) !== expected) {
      throw new Error(`compareVersions(${left}, ${right}) must be ${expected}, got ${host.compareVersions(left, right)}`)
    }
  }
  ...
  console.log(`version helpers OK: ${cases.length} semver cases + compareVersions ordering + npmrc registry parsing (no token leakage)`)
}
...
// Runtime snapshot helpers
{
  ...
  // dsh version resolves from the real CLI entry of the installed package
  // (located through the shared dependency anchors, never a cache path
  // hardcoded here). Where no dsh install is reachable at all this one check
  // is reported as skipped — reading a path that may not exist would crash the
  // whole harness instead.
  const dshPkgPath = resolveUiModule('@deepseek-ai/dsh/package.json', import.meta.url)
  if (dshPkgPath === undefined || readFileSync(dshPkgPath, 'utf8').length === 0) {
    console.log(`host snapshot OK: pid=${info.pid} ports=[${info.ports}] (dsh version check skipped: no @deepseek-ai/dsh package readable from the known anchors)`)
  } else {
    const dshRoot = dirname(dshPkgPath)
    const binPath = existsSync(join(dshRoot, 'lib', 'bin.js')) ? join(dshRoot, 'lib', 'bin.js') : dshPkgPath
    const expectedVersion = JSON.parse(readFileSync(dshPkgPath, 'utf8')).version
...
// - `routeEvents` is one ordered log of route registrations AND releases, which
//   is how "the health route is registered FIRST, before every optional step
//   and before /app" and "an unload releases them in reverse" are asserted;
// - `injectLists` / `settingsInjectAttempts` / `settingsReads` record every way
//   this plugin could still ask for a service dynamically. It declares both of
//   its services statically now and owns no settings namespace (the idle
//   auto-stop was its only consumer), so all three must stay empty/zero;
// - `hostEffects` keeps every `ctx.effect` label + disposer in registration
//   order, the way Cordis does before it runs them LIFO on unload;
 ...
 * instead of "registering a channel" into a variable nothing reads. The
 * service is not even declared in `host.inject` any more, so a real activation
 * would never hand it out.
 */
...
/**
 * The mock of ONE activation of this plugin. `agents` and `webServer` are the
 * module's STATIC `inject` dependencies, so Cordis exposes them to `apply` as
 * plain context properties and activates the plugin only once both are bound —
 * the whole transport therefore registers during `apply()`, on the plugin's own
 * fiber, with no child fiber in between.
 */
const hostCtx = {
  ...
  get: (name) => {
    ...
    // Optional reads for services this plugin neither declares nor uses. A real
    // Cordis answers `undefined` here; counting them keeps the removed settings
    // namespace from coming back through a read instead of an inject.
    if (name === 'settings') {
      settingsReads += 1
      return undefined
    }
  ...
  effect: (execute, label) => {
    const entry = { label: typeof label === 'string' ? label : null, dispose: null }
    hostEffects.push(entry)
    const dispose = execute()
    if (typeof dispose === 'function') entry.dispose = dispose
    return dispose
  },
  // Nothing in this plugin asks for a service dynamically any more, so every
  // ask is recorded and reported by section 2 rather than honoured.
  inject: (services) => {
    const list = services.join(',')
    injectLists.push(list)
    if (list.includes('settings')) settingsInjectAttempts += 1
    return undefined
  },
}
 ...
 * and hands back a real disposer, so a release is as observable as a
 * registration (the lifecycle section asserts the order they run in).
 */
const makeWebServerStub = () => {
  ...
  return {
    routes,
    /** Paths in the order their route disposers actually ran. */
    disposed,
    register: (route) => {
      ...
      return () => {
        ...
        routeEvents.push(`unroute:${entry.path}`)
      }
    },
  }
}
...
// 2) the removed settings surface, pinned: whatever the environment offers, this
//    plugin must neither register a namespace nor ask for one — the idle
//    auto-stop was its only consumer. Its two services are STATIC dependencies
//    now, so it also asks for nothing dynamically any more.
{
  hostWarns = []
  routeEvents = []
  webServerStub = makeWebServerStub()
  ...
  if (settingsReads !== 0) {
    throw new Error(`the host half must not even read a settings service any more, read ${settingsReads} time(s)`)
  }
  if (injectLists.length !== 0) {
    throw new Error(`a static inject means no dynamic ask at all, got ${JSON.stringify(injectLists)}`)
  }
...
//     exact `/ui-settings-other/health` that reports what actually attached.
{
  ...
  // Every side effect is owned by a labelled `ctx.effect`, in registration
  // order, and each hands Cordis a disposer to run on unload (Cordis runs them
  // LIFO — the lifecycle section pins the release order that produces).
  const expectedEffects = [
    'ui-settings-other: health route',
    'ui-settings-other: restart watchdog',
    'ui-settings-other: favicon route',
    'ui-settings-other: /app rpc route',
  ]
  const effectLabels = hostEffects.map((entry) => entry.label)
  if (JSON.stringify(effectLabels) !== JSON.stringify(expectedEffects)) {
    throw new Error(`effects must register in order ${JSON.stringify(expectedEffects)}, got ${JSON.stringify(effectLabels)}`)
  }
  const undisposables = effectLabels.filter((_, index) => typeof hostEffects[index].dispose !== 'function')
  if (undisposables.length !== 0) throw new Error(`every effect must hand back a disposer: ${JSON.stringify(undisposables)}`)
  console.log(`host health OK: exact ${HEALTH_PATH} (json + no-store) reports channel/branding, registered first; ${effectLabels.length} labelled effects, all disposable`)
}
...
//     fallback copy instead of the plugin taking the boot down). The carrier IS
//     a static dependency now, so production never reaches this state — it is
//     the kept "declared yet absent" fallback, and this is the only place that
//     branch is executed.
{
  hostWarns = []
  routeEvents = []
...
// 3) lifecycle: no restart watchdog outliving the plugin, and a transport that
//    lives (and dies) with the plugin's own fiber.
//
//    The mock mirrors the Cordis semantics these guarantees rest on: the two
//    STATIC `inject` services are already bound properties of the context, so
//    `apply` registers the routes itself instead of from a child fiber that
//    settles on a later microtask, and a fiber releases its effects in REVERSE
//    registration order (LIFO).
{
  ...
  // released by the watchdog, and the timer must not outlive the plugin).
  {
    ...
    // Registered on THIS fiber, during apply() — no microtask in between, which
    // is what the static `inject` buys: a hot reload remounts the transport
    // instead of waiting for a child fiber that a reload never re-activates.
    appRouteOf(web) // the transport of this case: one prefix route on webServer
    if (app.ctx.children.length !== 0) {
      throw new Error(`apply() must register on its own fiber, created ${app.ctx.children.length} child fiber(s)`)
    }
    if (JSON.stringify(web.routes.map((r) => `${r.kind} ${r.path}`)) !== JSON.stringify(EXPECTED_ROUTE_SURFACE)) {
      throw new Error(`the whole route surface must exist as soon as apply() returns, got ${JSON.stringify(web.routes.map((r) => r.path))}`)
    }
    await flush()
    ...
    try {
      ...
      // Cordis disposes effects in REVERSE registration order: the /app route
      // goes first, then the favicon, then the watchdog, then the diagnostics
      // route — which stays answerable for as long as anything else still is.
      const expectedRelease = [APP_PATH, FAVICON_PATH, HEALTH_PATH].join('|')
      if (web.disposed.join('|') !== expectedRelease) {
        throw new Error(`an unload must release the routes in reverse order ${expectedRelease}, got ${JSON.stringify(web.disposed)}`)
      }
      if (web.routes.length !== 0) {
        throw new Error(`an unload must leave no live route behind: ${JSON.stringify(web.routes.map((r) => r.path))}`)
      }
      console.log('lifecycle OK: routes registered on the plugin fiber, watchdog armed once, and both released by the unload (reverse order)')
    } finally {
...
// theme. Skipped (with a notice) when the theme bundle cannot be read.
{
  ...
  // Located through the shared dependency anchors (repo node_modules →
  // $NODE_PATH → the deployed profile trees), never a hardcoded install path.
  const themeCandidates = []
  const themePkg = resolveUiModule('@deepseek-ai/dsh-client-ui-theme/package.json', import.meta.url)
  if (themePkg !== undefined) themeCandidates.push(join(dirname(themePkg), 'lib', 'client.js'))
