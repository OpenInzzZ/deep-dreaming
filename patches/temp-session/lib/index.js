 ...
 * `webServer` once `webServer` AND `workspaceRegistry` are available — both
 * are declared in the `ctx.inject` list, so the child fiber waits for the HTTP
 * carrier instead of racing it. NEVER return the
 * `ctx.inject(...)` value from `apply` (Cordis treats a thenable return as an
 * invalid Effect); the route effect is registered on that child fiber, so
 * unloading / hot reloading the plugin unregisters `/temp-session` instead of
 * leaving a stale handler behind.
 */
export function apply(ctx, config = {}) {
  ...
  // rows in parallel and the HTTP carrier is bound later than
  // `workspaceRegistry`, so reading it with `ctx.get('webServer')` during this
  // callback races that binding, sees `undefined`, and silently skips the
  // page's whole transport. That race is exactly how four of five migrated
  // patches came up dead after the first restart; waiting on the carrier here
  // (instead of probing for it up front) makes the registration unconditional.
  ctx.inject(['webServer', 'workspaceRegistry'], (inner) => {
    const workspaceRegistry = inner.workspaceRegistry
    ...
    // would exist and the button would land on the SPA fallback. See
    // createRpcRoute() for the fence that replaces the Connection's own.
    //
    // The carrier and the effect are both read from the CHILD fiber: the route
    // is only useful while this fiber's registry is live, so it must be
    // released with it (`inner.effect`), and `inner.get` is the optional read
    // on that fiber — the declared dependency above is what makes the value
    // real, and this branch is the unreachable "declared yet somehow still
    // absent" fallback.
    const webServer = inner.get('webServer')
    if (webServer === undefined) {
      inner.logger?.warn('[temp-session] webServer is unavailable; the page cannot reach this half')
      return undefined
    }
    const handle = (endpoint, payload) => handleEndpoint(endpoint, payload, { workspaceRegistry, config, enqueue })
    inner.effect(() => webServer.register(createRpcRoute('/temp-session', handle)), 'temp-session: /temp-session rpc route')
    return undefined
  })
}
