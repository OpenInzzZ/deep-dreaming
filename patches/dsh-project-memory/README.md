...
每次调用在会话里渲染为**可折叠的「记忆」卡片**(类似思维链/终端执行的
折叠行)——折叠头显示「记忆」徽标、动作名(记忆 · 保存/检索/浏览)与一行
摘要(保存后显示「已保存:<标题>」,检索显示命中条目);展开可见完整参数
与结果全文;执行中的卡片自动展开。会话结束的记忆回顾因此以紧凑的
「记忆阶段」呈现,与普通消息/工具卡区分开。修改 `client.js` 后需重启
dsh web 生效(与其它补丁源码一致)。
