...
2026-09-18 实测(dsh 0.1.5-rc.2):一次 patch 文件写入之后,本补丁的 `/queue`
通道在运行中的进程里消失,而客户端半照常渲染 —— 表现是排队消息**拖拽排序
失效**,并弹出「排序失败:这条消息可能已经开始发送。」。真实原因是
`POST /queue/reorder` 落到 **404**(通道没注册),不是消息被抢发:
...
重排逻辑本身在 live agent 上验证过是正确的(定位 `next-turn` 项 → 两次
`splice` 交换 → 顺序正确),所以出现该提示时先怀疑 host 半是否在册。
