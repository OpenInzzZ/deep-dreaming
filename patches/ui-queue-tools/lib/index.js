...
export function apply(ctx) {
  ...
  // reading it with `ctx.get('webServer')` inside this callback races that
  ...
  ctx.inject(['webServer', 'agents'], (ctx) => {
    ...
    // would exist and the drag would land on the SPA fallback.
    // See createRpcRoute() for the fence that replaces the Connection's own.
    //
    // The carrier is read with `ctx.get` only as a guard: the declared
    // dependency above is what makes the value real, and this branch is the
    // unreachable fallback of "declared yet somehow still absent".
    const webServer = ctx.get('webServer')
    if (webServer === undefined) {
      ctx.logger?.warn('[ui-queue-tools] webServer is unavailable; the page cannot reach this half')
      return undefined
    }
    ctx.effect(() => webServer.register(createRpcRoute('/queue', handle)), 'ui-queue-tools: /queue rpc route')
    return undefined
  })
}
