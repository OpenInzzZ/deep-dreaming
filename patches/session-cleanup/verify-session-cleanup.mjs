 ...
 * outside the connection package, and with `webServer` DECLARED in the
 * `ctx.inject` list so activation waits for the carrier instead of racing it
 * with an optional `ctx.get` read) is owned by `ctx.effect` and must survive a
 * throwing settings wiring.
 *
...
// DOM deps resolve from the patch dir first; a DSH reinstall can prune the
// profile's hoisted copies (dangling links), so the harness checkout backs it up.
const harnessRequire = createRequire('D:/GitHub/deepseek-harness/apps/web/package.json')
const requireDom = (spec) => {
  try { return require(spec) } catch { return harnessRequire(spec) }
}
...
function makeCtx({ withSettings, settingsThrows = false, withWebServer = true }) {
  const calls = {
    info: [],
    warn: [],
    registered: null,
    ...
    settingsUpdates: [],
    settingsReplaces: [],
    /** Every dependency list the host half declared, in order. */
    injectLists: [],
  ...
  const ctx = {
    ...
    web,
    effect,
    get: (name) => (name === 'webServer' && withWebServer ? web.service : undefined),
    inject: (services, callback) => {
      const list = services.join(',')
      calls.injectLists.push(list)
      // The transport gate now DECLARES the web carrier: `webServer` is listed
      // first, so Cordis activates this callback only once the carrier is
      // bound — waiting instead of racing it with an optional `ctx.get` read
      // that would see `undefined` and silently drop the whole page transport.
      if (list === 'webServer,sessions') {
        return callback({
          logger,
          calls,
          effect,
...
// 1) no settings service -> entry config drives the cleanup, the route still works
{
  ...
  await settle()
  // The web carrier is a DECLARED dependency of the transport gate, so the
  // callback can only run once `webServer` exists — the fix for the
  // activation race that silently skipped the route after a restart.
  if (!ctx.calls.injectLists.includes('webServer,sessions')) {
    throw new Error(`the transport gate must DECLARE webServer: ${JSON.stringify(ctx.calls.injectLists)}`)
  }
...
let JSDOM = null
try {
  JSDOM = requireDom('jsdom').JSDOM
} catch { /* render section skipped below */ }

const React = requireDom('react')
...
const requireTable = (spec) => {
  ...
  if (spec === 'react/jsx-runtime') return requireDom('react/jsx-runtime')
...
if (JSDOM === null) {
  ...
  const { act } = React
  const { createRoot } = requireDom('react-dom/client')
